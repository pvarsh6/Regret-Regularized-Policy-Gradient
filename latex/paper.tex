...
% Appendix figure defaults

\newcommand{\appfigwidth}{0.8\linewidth}
\newcommand{\appfigvsep}{1.0em}
\newcommand{\appcurvewidth}{1.0\linewidth}
...
\item We demonstrate that RRPG achieves 95-96\% lower average external regret compared to vanilla policy gradient against adaptive opponents, while also reducing exploitability by 18-40\% and exhibiting more stable action probabilities (\cref{sec:results}).
...
Code and experiment outputs are available at \url{https://github.com/pvarsh6/Regret-Regularized-Policy-Gradient}.
...
The key hyperparameter is the regret-matching coefficient $\lambda_R$. We conduct a sweep over $\lambda_R \in \{0.05, 0.1, 0.2, 0.5, 1.0, 2.0\}$ and find that $\lambda_R = 2.0$ provides the best performance at our 10,000-round horizon. Notably, shorter preliminary runs (5,000 rounds) favored $\lambda_R = 0.1$ (see \cref{sec:ablations:lambda}), indicating that the optimal regularization strength can be horizon-dependent.
...
\paragraph{Primary metric: Mean per-round external regret.}
Our primary metric is \emph{mean per-round external regret}, i.e., a time-averaged regret quantity. Depending on context, we either report the full-horizon time-averaged external regret $R_T / T$ or its final-window counterpart (mean per-round external regret in the final evaluation window). This is our main metric, as it directly captures the no-regret property (lower is better, and no-regret implies it approaches 0 as $T$ grows).
...
Exploitability is reported only for zero-sum games and serves as a complementary equilibrium-based metric.
...
Each experiment runs for $T = 10{,}000$ rounds. We average results over 20 random seeds to account for stochasticity in both network initialization and opponent randomness. Unless otherwise noted, we report mean $\pm$ standard deviation across seeds.

For steady-state analysis, we compute metrics over the final $K = 1{,}000$ rounds, which we refer to as the \emph{evaluation window}. In particular, when we report regret ``in the final 1000 rounds,'' we mean the \emph{mean per-round external regret in this final evaluation window} (a final-window time-averaged regret quantity), not a cumulative sum. This captures performance after initial transients have settled.
...
Representative regret curves are provided in Appendix~\ref{app:baselines} (MW: \cref{fig:baseline_comparison_mw}; EXP3: \cref{fig:baseline_comparison_exp3}).

\section{Results}
\label{sec:results}

\subsection{Regret Behavior (Primary Claim)}
\label{sec:results:regret}

We first evaluate the primary claim: regret-regularized policy gradients substantially improve no-regret behavior and stability, especially against adaptive opponents. Unless otherwise noted, reported regret values are \emph{mean per-round external regret in the final evaluation window} (last 1000 rounds).

\subsubsection{Adaptive Opponents}

\cref{tab:adaptive_summary} quantifies performance against adaptive opponents using metrics from the final evaluation window (last 1000 rounds; steady-state behavior).

\begin{table}[H]
\caption{Performance against adaptive opponents (final evaluation window: last 1000 rounds; mean $\pm$ std over 20 seeds). ``Ext. regret'' is mean per-round external regret in the final window. Exploitability is reported only for zero-sum games. Lower is better for both metrics.}
\label{tab:adaptive_summary}
\centering
\small
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Algorithm} & \textbf{Ext. regret} & \textbf{Exploitability} \\
\midrule
\multicolumn{3}{c}{\emph{Matching Pennies vs. MW}} \\
\midrule
MW & 0.01 $\pm$ 0.00 & 0.95 $\pm$ 0.02 \\
PG-Vanilla & 1.03 $\pm$ 0.71 & 0.97 $\pm$ 0.08 \\
PG-RM+ 2.0 & \textbf{0.05 $\pm$ 0.01} & \textbf{0.80 $\pm$ 0.03} \\
\midrule
\multicolumn{3}{c}{\emph{Rock-Paper-Scissors vs. MW}} \\
\midrule
MW & 0.01 $\pm$ 0.00 & 0.94 $\pm$ 0.05 \\
PG-Vanilla & 0.86 $\pm$ 0.73 & 0.89 $\pm$ 0.14 \\
PG-RM+ 2.0 & \textbf{0.04 $\pm$ 0.01} & \textbf{0.53 $\pm$ 0.11} \\
\bottomrule
\end{tabular}
\end{table}

Against MW in Matching Pennies, classical MW achieves low final-window mean per-round external regret ($0.01 \pm 0.00$) as expected. PG-Vanilla incurs substantial final-window regret ($1.03 \pm 0.71$), indicating failure to achieve no-regret behavior. The high standard deviation reflects instability across random seeds.

RRPG with $\lambda_R = 2.0$ dramatically reduces final-window regret to $0.05 \pm 0.01$, a \textbf{96\% improvement over vanilla PG}. Exploitability decreases to $0.80 \pm 0.03$, an 18\% improvement. While RRPG does not match MW's theoretical guarantee (which would imply $R_T/T = O(\sqrt{T\log n}/T) \approx 0.003$ at $T=10{,}000$), it substantially narrows the gap between vanilla neural policy gradients and classical no-regret methods.

Results for Rock-Paper-Scissors show similar patterns. PG-Vanilla has high final-window regret ($0.86 \pm 0.73$), suggesting that the three-action setting exacerbates cycling/instability in vanilla PG. PG-RM+ reduces regret to $0.04 \pm 0.01$ (\textbf{96\% improvement}) and exploitability to $0.53 \pm 0.11$ (40\% improvement).

\paragraph{Horizon-dependent hyperparameter selection.}
Notably, we initially tuned $\lambda_R$ on shorter runs (5,000 rounds) where $\lambda_R = 0.1$ performed best. However, extending to our full 10,000-round horizon revealed that stronger regularization ($\lambda_R = 2.0$) achieves superior asymptotic performance. This indicates $\lambda_R$ is horizon-dependent: stronger regret regularization becomes increasingly beneficial for longer interactions. This finding strengthens our main claim, as it shows RRPG's advantage grows with extended play. We report $\lambda_R = 2.0$ throughout main results.

Overall, across both games, regret regularization yields substantially lower final-window regret against adaptive opponents. We discuss strategic-quality metrics (exploitability and equilibrium deviation) separately in \cref{sec:results:strategic}.

\noindent Complete metric breakdowns (ER+, exploitability, instant regret) for all algorithms are provided in Appendix~\ref{app:performance} (Matching Pennies: Appendix~\ref{app:adaptive_mp_metrics}; Rock-Paper-Scissors: Appendix~\ref{app:adaptive_rps_metrics}).

\subsubsection{Static Opponents}
...
\begin{table}[H]
\caption{Performance against static opponents (final evaluation window: last 1000 rounds). ``Ext. regret'' is mean per-round external regret in the final window. All methods eventually exploit static strategies.}
...
PG-RM+ 2.0 & 0.0004 $\pm$ 0.0002 & 1.00 $\pm$ 0.00 \\
\midrule
...
PG-RM+ 2.0 & 0.001 $\pm$ 0.0004 & 0.998 $\pm$ 0.002 \\
\bottomrule
\end{tabular}
\end{table}
...
\noindent Full learning dynamics against static opponents are provided in Appendix~\ref{app:static_dynamics}.
...
\begin{table}[H]
\caption{Policy entropy (final evaluation window: last 1000 rounds) for Matching Pennies vs MW. Higher entropy indicates more uniform (less exploitable) policies.}
...
PG-RM+ 2.0 & \textbf{0.25 $\pm$ 0.04} \\
Optimal (uniform) & 0.693 \\
\bottomrule
\end{tabular}
\end{table}

PG-RM+ maintains higher entropy (0.25) compared to PG-Vanilla (0.05), though both are far below the theoretical optimum (0.693 for uniform distribution over 2 actions). Qualitative inspection of individual trajectories reveals that PG-RM+ oscillations are damped and centered around the Nash equilibrium, while PG-Vanilla oscillations are often larger in amplitude and slower to converge.

This stability translates to lower variance in performance metrics across seeds. The coefficient of variation (std/mean) for final-window mean per-round external regret is 69\% for PG-Vanilla vs. 33\% for PG-RM+ against MW opponents, indicating more consistent performance.

\subsection{Strategic Quality (Secondary Claim)}
\label{sec:results:strategic}

We next report \emph{strategic quality} as a complementary claim. In our two-player zero-sum games, exploitability serves as an equilibrium-based notion of ``Nash deviation'': lower exploitability indicates the learned strategy is harder to exploit and closer to Nash equilibrium. As shown in \cref{tab:adaptive_summary}, RRPG reduces exploitability relative to vanilla PG (e.g., from $0.97$ to $0.80$ in Matching Pennies vs.\ MW and from $0.89$ to $0.53$ in RPS vs.\ MW in the final evaluation window), indicating improved strategic quality alongside regret reduction.

\noindent Additional baseline-comparison curves are provided in Appendix~\ref{app:baselines}.
...
\begin{table}[H]
\caption{Effect of regret-matching coefficient $\lambda_R$ (Matching Pennies vs. MW, final evaluation window: last 1000 rounds).}
...
0.1 & 0.056 $\pm$ 0.019 & 0.85 $\pm$ 0.12 & 0.21 $\pm$ 0.12 \\
...
\textbf{2.0} & \textbf{0.047 $\pm$ 0.007} & \textbf{0.80 $\pm$ 0.03} & \textbf{0.25 $\pm$ 0.04} \\
\bottomrule
\end{tabular}
\end{table}
...
While $\lambda_R = 0.1$ performed best in preliminary 5,000-round experiments, our full 10,000-round runs show $\lambda_R = 2.0$ achieves lower final-window regret (0.047 vs 0.056) and exploitability (0.80 vs 0.85). This demonstrates that optimal $\lambda_R$ increases with horizon length.

\noindent Full learning curves for the $\lambda_R$ sweep are provided in Appendix~\ref{app:lambda_sweep}.
...
\begin{table}[H]
\caption{Functional form variants (Matching Pennies vs. MW, final evaluation window: last 1000 rounds).}
\label{tab:variants}
\centering
\small
\begin{tabular}{@{}lcc@{}}
...
\midrule
RM+ (standard) & 0.047 $\pm$ 0.007 & 0.80 $\pm$ 0.03 \\
...
\textbf{RM+ (clipping negative regrets)} performs best with average regret of 0.047. The clipping operation appears to provide a useful inductive bias by completely discarding actions that have underperformed, focusing probability mass on actions with positive regret.
...
\noindent Full learning curves for functional form variants are provided in Appendix~\ref{app:functional_forms}.
...
\midrule
Cumulative (standard) & 0.047 $\pm$ 0.007 \\
...
Discounting with $\gamma=0.95$ achieves notably lower regret (0.019 vs. 0.047), suggesting that weighting recent observations more heavily can reduce sensitivity to early-game noise when policies are still far from optimal. The $\gamma=0.99$ variant is intermediate.
...
\noindent Full learning curves for discounted regret variants are provided in Appendix~\ref{app:discounting}.
...
\midrule
Constant $\lambda_R=2.0$ & 0.047 $\pm$ 0.007 \\
...
Both schedules achieve slightly worse regret (0.048--0.051) compared to constant $\lambda_R$ (0.047), though the differences are small. Schedules may be beneficial for balancing initial regularization (to avoid early instability) with later flexibility (to fine-tune against specific opponents).
...
\noindent Full learning curves for progressive schedules are provided in Appendix~\ref{app:schedules}.
...
\begin{itemize}
\item \textbf{PG-RM+only}: $\lambda_R = 2.0$, $\lambda_H = 0$
\item \textbf{PG-Entropyonly}: $\lambda_R = 0$, $\lambda_H = 0.1$
\item \textbf{PG-RM+H (standard)}: $\lambda_R = 2.0$, $\lambda_H = 0.01$
\end{itemize}
...
\midrule
RM+only & 0.037 $\pm$ 0.012 & 0.045 $\pm$ 0.011 \\
Entropyonly & 0.582 $\pm$ 0.478 & 0.582 $\pm$ 0.478 \\
RM+H (standard) & \textbf{0.047 $\pm$ 0.007} & \textbf{0.044 $\pm$ 0.014} \\
\bottomrule
\end{tabular}
\end{table}

The results are striking: \textbf{regret regularization is the key driver of performance improvement}. RM+only achieves very low regret (0.037--0.045) even without entropy regularization. In contrast, Entropyonly (standard PG with high entropy bonus) achieves poor performance (0.582 regret)---only slightly better than vanilla PG.

Combining both (RM+H) achieves good performance (0.047 vs MW, 0.044 vs EXP3) with reasonable computational cost. The mild entropy regularization ($\lambda_H=0.01$) helps with exploration without dominating the regret signal.
...
\noindent Additional component-ablation learning curves are provided in Appendix~\ref{app:components}.
...
\begin{itemize}
\item \textbf{95-96\% lower external regret} compared to vanilla policy gradient against adaptive opponents
\item \textbf{18-40\% lower exploitability}, indicating closer approximation to Nash equilibrium
...
This appendix provides comprehensive experimental results supporting the main paper, including full performance comparisons, detailed ablation studies, and diagnostic metrics.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Adaptive Opponents: Matching Pennies}
\label{app:adaptive_mp_metrics}

\begin{figure}[H]
    \centering
    \includegraphics[width=\appfigwidth]{figures/adaptive/bar__MP__ER+.png} \\[\appfigvsep]
    \includegraphics[width=\appfigwidth]{figures/adaptive/bar__MP__EXP.png} \\[\appfigvsep]
    \includegraphics[width=\appfigwidth]{figures/adaptive/bar__MP__IR.png} \\[\appfigvsep]
    % \includegraphics[width=\appfigwidth]{figures/adaptive/bar__MP__TAEXP.png}
    \caption{Complete performance metrics for Matching Pennies against adaptive opponents (MW and EXP3). From top to bottom: (a) External regret, (b) Exploitability, (c) Instant regret. All 18 algorithms shown with 95\% confidence intervals over 20 seeds.}
    \label{fig:mp_adaptive_complete}
\end{figure}

\clearpage

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Adaptive Opponents: Rock-Paper-Scissors}
\label{app:adaptive_rps_metrics}

\begin{figure}[H]
    \centering
    \includegraphics[width=\appfigwidth]{figures/adaptive/bar__RPS__ER+.png} \\[\appfigvsep]
    \includegraphics[width=\appfigwidth]{figures/adaptive/bar__RPS__EXP.png} \\[\appfigvsep]
    \includegraphics[width=\appfigwidth]{figures/adaptive/bar__RPS__IR.png} \\[\appfigvsep]
    % \includegraphics[width=\appfigwidth]{figures/adaptive/bar__RPS__TAEXP.png}
    \caption{Complete performance metrics for Rock-Paper-Scissors against adaptive opponents. From top to bottom: (a) External regret, (b) Exploitability, (c) Instant regret. RRPG variants consistently achieve lower regret and exploitability.}
    \label{fig:rps_adaptive_complete}
\end{figure}

\clearpage

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Static Opponents: Matching Pennies}
\label{app:static_mp_metrics}

\begin{figure}[H]
    \centering
    \includegraphics[width=\appfigwidth]{figures/static/bar__MP__ER+.png} \\[\appfigvsep]
    \includegraphics[width=\appfigwidth]{figures/static/bar__MP__EXP.png} \\[\appfigvsep]
    \includegraphics[width=\appfigwidth]{figures/static/bar__MP__IR.png} \\[\appfigvsep]
    % \includegraphics[width=\appfigwidth]{figures/static/bar__MP__TAEXP.png}
    \caption{Complete performance metrics for Matching Pennies against static opponents (Deterministic, Biased, Uniform). From top to bottom: (a) External regret, (b) Exploitability, (c) Instant regret. All learning algorithms eventually exploit static strategies.}
    \label{fig:mp_static_complete}
\end{figure}

\clearpage

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Static Opponents: Rock-Paper-Scissors}
\label{app:static_rps_metrics}

\begin{figure}[H]
    \centering
    \includegraphics[width=\appfigwidth]{figures/static/bar__RPS__ER+.png} \\[\appfigvsep]
    \includegraphics[width=\appfigwidth]{figures/static/bar__RPS__EXP.png} \\[\appfigvsep]
    \includegraphics[width=\appfigwidth]{figures/static/bar__RPS__IR.png} \\[\appfigvsep]
    % \includegraphics[width=\appfigwidth]{figures/static/bar__RPS__TAEXP.png}
    \caption{Complete performance metrics for Rock-Paper-Scissors against static opponents. From top to bottom: (a) External regret, (b) Exploitability, (c) Instant regret. Regret regularization does not impair performance in non-adaptive settings.}
    \label{fig:rps_static_complete}
\end{figure}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Regret-Matching Coefficient $\lambda_R$}
\label{app:lambda_sweep}

\begin{figure}[H]
    \centering
    \includegraphics[width=\appcurvewidth]{figures/adaptive/curve__MP__MW__ER__rmplus_lambda_sweep.png} \\[\appfigvsep]
    \includegraphics[width=\appcurvewidth]{figures/adaptive/curve__RPS__MW__ER__rmplus_lambda_sweep.png}
    \caption{Effect of regret-matching coefficient $\lambda_R \in \{0.05, 0.1, 0.2, 0.5, 1.0, 2.0\}$ on average external regret over time. Top: Matching Pennies vs MW. Bottom: Rock-Paper-Scissors vs MW. All values in $[0.1, 2.0]$ substantially outperform vanilla PG ($\lambda_R = 0$). Stronger regularization ($\lambda_R = 2.0$) performs best at our 10,000-round horizon.}
    \label{fig:lambda_sweep}
\end{figure}

\paragraph{Horizon-dependent optimal regularization.}
\Cref{fig:lambda_sweep} demonstrates that optimal $\lambda_R$ is horizon-dependent. While $\lambda_R = 0.1$ performed best in preliminary 5,000-round experiments, extending to 10,000 rounds reveals that stronger regularization ($\lambda_R = 2.0$) achieves superior asymptotic performance. This suggests that longer interactions benefit from stronger alignment with regret-matching principles, while shorter horizons may prefer weaker regularization to maintain adaptation speed.

\clearpage

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Functional Form Variants}
\label{app:functional_forms}

\begin{figure}[H]
    \centering
    \includegraphics[width=\appcurvewidth]{figures/adaptive/curve__MP__MW__ER__functional_form.png} \\[\appfigvsep]
    \includegraphics[width=\appcurvewidth]{figures/adaptive/curve__RPS__MW__ER__functional_form.png}
    \caption{Comparison of functional form variants against MW opponent. Top: Matching Pennies. Bottom: Rock-Paper-Scissors. Variants shown: RM+ (standard clipping), RM-NoClip (shift to non-negative), and Softmax-RM with $\tau \in \{0.5, 1.0\}$. Standard RM+ achieves lowest regret, suggesting the clipping operation provides useful inductive bias.}
    \label{fig:functional_forms_mw}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\appcurvewidth]{figures/adaptive/curve__MP__EXP3__ER__functional_form.png} \\[\appfigvsep]
    \includegraphics[width=\appcurvewidth]{figures/adaptive/curve__RPS__EXP3__ER__functional_form.png}
    \caption{Functional form comparison against EXP3 opponent. Top: Matching Pennies. Bottom: Rock-Paper-Scissors. Results are consistent across opponent types, with RM+ clipping providing best performance.}
    \label{fig:functional_forms_exp3}
\end{figure}

\clearpage

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Discounted Regret}
\label{app:discounting}

\begin{figure}[H]
    \centering
    \includegraphics[width=\appcurvewidth]{figures/adaptive/curve__MP__MW__ER__discounting.png} \\[\appfigvsep]
    \includegraphics[width=\appcurvewidth]{figures/adaptive/curve__RPS__MW__ER__discounting.png}
    \caption{Comparison of cumulative vs discounted regret with $\gamma \in \{0.95, 0.99\}$. Top: Matching Pennies vs MW. Bottom: Rock-Paper-Scissors vs MW. Discounting achieves notably lower regret, suggesting benefit from down-weighting early-game noise when policies are far from optimal.}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Progressive Schedules}
\label{app:schedules}

\begin{figure}[H]
    \centering
    \includegraphics[width=\appcurvewidth]{figures/adaptive/curve__MP__MW__ER__schedules.png} \\[\appfigvsep]
    \includegraphics[width=\appcurvewidth]{figures/adaptive/curve__RPS__MW__ER__schedules.png}
    \caption{Comparison of constant $\lambda_R = 2.0$ vs progressive decay schedules (linear and exponential). Top: Matching Pennies vs MW. Bottom: Rock-Paper-Scissors vs MW. Constant regularization performs best in our stationary setting, but schedules may be beneficial in non-stationary environments.}
    \label{fig:schedules}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\appcurvewidth]{figures/adaptive/curve__MP__MW__ER__ablation_components.png} \\[\appfigvsep]
    \includegraphics[width=\appcurvewidth]{figures/adaptive/curve__RPS__MW__ER__ablation_components.png}
    \caption{Ablation isolating regret regularization from entropy regularization against MW opponent. Top: Matching Pennies. Bottom: Rock-Paper-Scissors. RM+only ($\lambda_R = 2.0, \lambda_H = 0$) achieves very low regret, while Entropyonly ($\lambda_R = 0, \lambda_H = 0.1$) performs poorly. This confirms regret-matching regularization is the key driver of performance improvement.}
    \label{fig:component_ablation_mw}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\appcurvewidth]{figures/adaptive/curve__MP__EXP3__ER__ablation_components.png} \\[\appfigvsep]
    \includegraphics[width=\appcurvewidth]{figures/adaptive/curve__RPS__EXP3__ER__ablation_components.png}
    \caption{Component ablation against EXP3 opponent. Top: Matching Pennies. Bottom: Rock-Paper-Scissors. Results confirm that regret regularization provides qualitatively different signal than generic exploration bonuses.}
    \label{fig:component_ablation_exp3}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\appcurvewidth]{figures/adaptive/curve__MP__MW__ER__baseline_comparison.png} \\[\appfigvsep]
    \includegraphics[width=\appcurvewidth]{figures/adaptive/curve__RPS__MW__ER__baseline_comparison.png}
    ...
    \label{fig:baseline_comparison_mw}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\appcurvewidth]{figures/adaptive/curve__MP__EXP3__ER__baseline_comparison.png} \\[\appfigvsep]
    \includegraphics[width=\appcurvewidth]{figures/adaptive/curve__RPS__EXP3__ER__baseline_comparison.png}
    \caption{Baseline comparison against EXP3 opponent. Top: Matching Pennies. Bottom: Rock-Paper-Scissors. Comprehensive comparison showing all baseline algorithms and RRPG variants.}
    \label{fig:baseline_comparison_exp3}
\end{figure}

\clearpage

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Static Opponent Dynamics}
\label{app:static_dynamics}

\begin{figure}[H]
    \centering
    \includegraphics[width=\appfigwidth]{figures/static/curve__MP__Det__ER__baseline_comparison.png} \\[\appfigvsep]
    \includegraphics[width=\appfigwidth]{figures/static/curve__MP__Bias__ER__baseline_comparison.png} \\[\appfigvsep]
    \includegraphics[width=\appfigwidth]{figures/static/curve__MP__Uni__ER__baseline_comparison.png}
    \caption{Learning dynamics for Matching Pennies against static opponents. From top to bottom: (a) Deterministic opponent, (b) Biased opponent (60\% Heads), (c) Uniform random opponent. All methods eventually exploit deterministic and biased opponents. Against uniform random, all methods converge to near-zero regret.}
    \label{fig:static_dynamics_mp}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\appfigwidth]{figures/static/curve__RPS__Det__ER__baseline_comparison.png} \\[\appfigvsep]
    \includegraphics[width=\appfigwidth]{figures/static/curve__RPS__Bias__ER__baseline_comparison.png} \\[\appfigvsep]
    \includegraphics[width=\appfigwidth]{figures/static/curve__RPS__Uni__ER__baseline_comparison.png}
    \caption{Learning dynamics for Rock-Paper-Scissors against static opponents. From top to bottom: (a) Deterministic opponent, (b) Biased opponent, (c) Uniform random opponent. Consistent with Matching Pennies, regret regularization does not impair exploitation of fixed strategies.}
    \label{fig:static_dynamics_rps}
\end{figure}

\clearpage

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Additional Diagnostic Metrics}
\label{app:diagnostics}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Cumulative Regret}
\label{app:diagnostics:cum_regret}

\begin{figure}[H]
    \centering
    \includegraphics[width=\appcurvewidth]{figures/adaptive/curve__MP__MW__ERcum__baseline_comparison.png} \\[\appfigvsep]
    \includegraphics[width=\appcurvewidth]{figures/adaptive/curve__RPS__MW__ERcum__baseline_comparison.png}
    \caption{Cumulative external regret $R_T$ over time against MW opponent. Top: Matching Pennies. Bottom: Rock-Paper-Scissors. RRPG achieves sublinear growth similar to classical no-regret algorithms, while vanilla PG exhibits linear growth.}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Instant Regret (Oracle Bound)}
\label{app:diagnostics:instant_regret}

\begin{figure}[H]
    \centering
    \includegraphics[width=\appcurvewidth]{figures/adaptive/curve__MP__MW__IRavg__baseline_comparison.png} \\[\appfigvsep]
    \includegraphics[width=\appcurvewidth]{figures/adaptive/curve__RPS__MW__IRavg__baseline_comparison.png}
    \caption{Average cumulative instant regret (regret of best response at each round) against MW opponent. Top: Matching Pennies. Bottom: Rock-Paper-Scissors. This represents an oracle upper bound on achievable performance. All learning algorithms perform below this bound as expected.}
    \label{fig:instant_regret}
\end{figure}
